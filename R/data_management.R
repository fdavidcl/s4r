dataset_list <- function() {
  ...
  datasets$IsoletVowels <- function() read_data("data/isolettrain.data") %>% class_last(c(1, 5, 9, 15, 21))

  datasets$Jasmine <- function() read.csv("data/jasmine.csv") %>% class_first()

  datasets$Madelon <- function() cbind(read.table("data/madelon_train.data"), class = read.table("data/madelon_train.labels")) %>% class_last()
